...
We usually need to use Docker when we need to do DevOps, and Docker is currently the most widely used containerization technology. At the same time, we may also use cloud computing power to help us complete these tasks, so today we will introduce how to install Docker on [Amazon Linux 2](https://aws.amazon.com/amazon-linux-2/).

## Connect to EC2

We can use [Amazon EC2](https://aws.amazon.com/ec2/) to create a virtual machine, and we will use Amazon Linux 2 as our operating system. After creating the EC2 instance, we can connect to it via SSH.

you should change `your.pem` to your own private key file, and `<ec2-public-ip>` to your EC2 instance's public IP address.
